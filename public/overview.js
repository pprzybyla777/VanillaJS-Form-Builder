...
// Genereting HTML
function generateOverviewItemHTML(question) {
    return `
  <ul>
  <li id="${question.id}">
    Question:<span class="question">${question.name}</span>  Type:<span class="type">${question.type}</span>
...
function generateOverviewSubitemHTML(subinput) {
    return `
    <ul>
...
export function showOverview() {
    ...
    overviewContainer.innerHTML = Object.values(data.questions)
        .map((question) => generateOverviewItemHTML(question))
        .join("");
